...
| `transcription`           | Emitted when the agent's response has been finalized. Includes the field `transcription` that contains the transcription.      |
...
useDailyEvent(
  ...
  useCallback((ev: DailyEventObjectAppMessage) => {
    ...
    if (ev?.data?.type === "transcription") {
      console.log("The latest agent's response transcription: ", ev?.data?.transcription);
    }
