...
| :------------ | :--------------------------------------------------------------------------------------- |
| `chat-msg`    | Send a message to the agent for it to respond to. Also supports triggering avatar image changes via /imagine. (see [Avatar image changes with `/imagine`](#imagine))                                       |
...
- - **Agent setup requirement:** *Allow users to modify appearance* must be enabled in the agent configuration  
  - This setting is **enabled by default for all newly created agents**