...
```
  ...
  <Note>
  When using the [**Gemini Live S2S**](https://docs.livekit.io/agents/models/realtime/plugins/gemini/) model for realtime interactions, set `response_done_timeout=0.5` to handle end of responses correctly.
  
  If you encounter stutters or glitches with any other TTS, please contact support@lemonslice.com.
  </Note>
  ...
  <Step title="Shutdown the LiveKit room">
    ...
    3. Call the [session control endpoint](/self-managed/endpoint/sessions) with the `terminate` event to shutdown only the LemonSlice avatar without shutting down the LiveKit room or agent.
    ...
    <Warning>If the LemonSlice session is not shut down, the LemonSlice session will remain active until the configured idle timeout is reached or the 30-minute maximum session duration expires.</Warning>
  </Step>
</Steps>
