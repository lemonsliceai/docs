...
```
  ...
  <Step title="Shutdown the LiveKit room">
    ...
    3. Call the [session control endpoint](/api-reference/control-session) with the `terminate` event to shutdown only the LemonSlice avatar without shutting down the LiveKit room or agent.
