...
<a href="https://lemonslice.com/promo" style={{ display: 'inline-block', padding: '10px 20px', backgroundColor: '#4CAF50', color: 'white', textDecoration: 'none', borderRadius: '5px' }}>Get 1 month free ($100)</a>
...
```
  ...
  <Step title="Shutdown the LiveKit room">
    ...
    Call `ctx.room.disconnect()` to close the LiveKit room connection.
    Or, call `ctx.shutdown()` to stop the Agent's JobContext if you don't want to shutdown the LiveKit room.
